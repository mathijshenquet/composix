...
- While waiting on anything (gates, agents, CI), always have a ~10-minute background timer armed so a wake-up is guaranteed; never idle longer than that. The timer is the FALLBACK — the primary wake signal is the per-worker watcher (see Worker C2 below).
...
3. `herdr agent prompt <name> '<spec pointer>'`. First-prompt swallow is common; the RELIABLE landing check is a second prompt — an `agent_working` refusal proves the first landed (pane reads give false negatives). Re-send once if the agent stays idle. NEVER discard prompt output to /dev/null: `agent prompt` REFUSES with `agent_working` while the agent is mid-turn, and a swallowed refusal reads as delivered (2026-08-05: a scope-trim bounced silently this way, caught by Mathijs). Mid-turn delivery, honestly: `herdr agent send <name> "<text>"` puts text in the composer only — a trailing `\n` is a literal newline (multiline composer), NOT submit, and scripted tab-to-queue is UNVERIFIED (2026-08-05: Mathijs queued the message by hand). Reliable options: wait for idle and `agent prompt` (reading its output), or ask Mathijs to queue the composer text from his attach. Always verify by pane read: delivered = the message sits above the working indicator, not behind `›`.
