...
3. `herdr agent prompt <name> '<spec pointer>'`. First-prompt swallow is common; the RELIABLE landing check is a second prompt — an `agent_working` refusal proves the first landed (pane reads give false negatives). Re-send once if the agent stays idle. NEVER discard prompt output to /dev/null: `agent prompt` REFUSES with `agent_working` while the agent is mid-turn, and a swallowed refusal reads as delivered (2026-08-05: a scope-trim bounced silently this way, caught by Mathijs). Mid-turn delivery, VERIFIED (2026-08-05 luna experiment): `herdr agent send <name> "<text>"` puts text in the composer (a `\n` is a literal newline, not submit), then `herdr agent send <name> "$(printf '\t')"` queues it — pane shows "Queued follow-up inputs ↳ <text>"; it does NOT interrupt the running turn and is consumed at the next turn boundary. Verify by pane read for the Queued marker; without it the text sits unsubmitted behind `›`.
