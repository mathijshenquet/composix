...
BUILDER build memo miss 1ef44683a067 -> /nix/store/…-cix-build-view
```
...
BUILDER build step 2 RUN executed
BUILDER build memo miss cd02c12d08fd -> /nix/store/…-cix-build-view
```
...
$ CIX_BUILD_WORKSPACE_DIR=$PWD/../.workspaces-proj1 cix build .
{"proj1-api":"/nix/store/…-cix-item-proj1-api","proj1-worker":"/nix/store/…-cix-item-proj1-worker"}
BUILDER build memo hit completed output (zero Nix subprocesses)
```
