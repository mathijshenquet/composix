...
| [bridge networks and service DNS](https://docs.docker.com/engine/network/drivers/bridge/) | ⏳ CIP-86 specifies optional group netns/pod mechanics, but compose-tree leg 1 rejects `network` rather than accepting an inert field | Per-container/group namespaces, bridge networks, user-defined networks, embedded DNS, aliases, and IPAM. |
...
| docker | disposition | still missing |
| --- | --- | --- |
| [Compose services](https://docs.docker.com/reference/compose-file/services/) | 🔁 strict `cixCompose: 1` group nodes use one recursive `children` map of fully-qualified item refs, tagged compose refs, and inline groups. Leaf options apply env/listener/dir/secret/schedule/SHM policy; flat `services` input is migration-refused (`crates/cix-compose/src/{model,resolve,generation}.rs`; `nix/scenarios/tree.nix`). | Docker Compose YAML compatibility, multi-file merging/includes, `extends`, device grants, and broad Compose-field coverage. Pod networking and publish climbing remain explicitly refused this leg. |
...
| [`depends_on` / ordering](https://docs.docker.com/reference/compose-file/services/#depends_on) | 🔁 group-local Unix `edges` require their setup unit and put the consumer `After=` the producer service while exposing only declared runtime paths; systemd start-job completion makes those structural consumers wait for producer readiness (`crates/cix-compose/src/generation.rs`; `nix/scenarios/health.nix`) | ❌ `condition: service_healthy` is deliberately rejected rather than creating a second health graph. Cross-group published surfaces, restart propagation, and network discovery remain. |
