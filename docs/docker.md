...
- **Networking between networked services has a pod-sized boundary, not Docker networks.** A
  compose group may claim `network: "pod"`: its nearest-member subtree shares one named netns and
  localhost, host publication uses socket FD passing or an in-netns proxyd, and declared egress
  gets stable bridge/veth addressing. Podless services deliberately keep the host stack. Named
  networks, service DNS/aliases, and `talks-to` segmentation remain D26/D27 work rather than being
  implied by the pod mechanism.
...
| [`image inspect`](https://docs.docker.com/reference/cli/docker/image/inspect/) | ✅ `cix inspect <ref>` (D35): stable JSON (or `--human`) reports store path, narHash, per-system outputs, validated parsed manifest, closure size, recorded trusted keys, upstream, and drvPath when present. Qualified refs use the same remote grammar as `cix pull` without creating a local mirror tag. | — |
...
| [`-p` port publish](https://docs.docker.com/reference/cli/docker/container/run/#publish) | 🔁 D24 compiles declared TCP or UDP ports to kernel-enforced `SocketBindAllow=`/`SocketBindDeny=`; unary v3 listeners bind through transient sockets, while compose pod publishes use a host `.socket` and either pass its TCP FD directly or run TCP `systemd-socket-proxyd` inside the pod (D29/D49; `scenario-netns`). | No Docker NAT or UDP compose publication: the fixed-port fallback is deliberately TCP proxyd-only. Unary ordinary ports still have no policy-level bind-address choice. |
| [`-v` / `--mount`](https://docs.docker.com/engine/storage/bind-mounts/) | ✅ cix-managed private data is a role dir at conventional or arbitrary absolute app paths; exact managed binds are allow-listed under the strict root, while `DIR` is supplied through strict compose `host:` or `shared:` backing, and `as:` changes only its lifecycle treatment. `cix run --dir` mirrors host/reclassification flags. | No Docker mount grammar, copy-up, anonymous volumes, or mount propagation vocabulary. |
| [`--restart` policies](https://docs.docker.com/engine/containers/start-containers-automatically/) | 🔁 declaring `LIVENESS` is an explicit restart opt-in: cix emits a fixed bounded `Restart=on-failure`/`RestartSec=`/start-limit policy around systemd's watchdog ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No Docker policy-name compatibility or per-service restart tuning; compose-mechanical tuning waits for a consumer. |
...
| [`stop`](https://docs.docker.com/reference/cli/docker/container/stop/) / [`kill`](https://docs.docker.com/reference/cli/docker/container/kill/) / signals | ✅ `systemctl stop`; item `STOPSIGNAL <signal>` maps to `KillSignal=`, and compose `stopTimeout` maps to `TimeoutStopSec=` ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No Docker CLI compatibility or arbitrary `kill` signal verb. |
...
| [`cp`](https://docs.docker.com/reference/cli/docker/container/cp/) | ❌ role directories are ordinary host paths, exposed by `cix inspect --runtime`; copy them directly ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No daemon tunnel through an immutable item/private namespace boundary. |
...
| [`restart`](https://docs.docker.com/reference/cli/docker/container/restart/) | 🔁 `systemctl restart`; policy tuning is later compose-mechanical work ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | — |
...
| [namespace modes (`--ipc`, `--pid`, `--uts`)](https://docs.docker.com/reference/cli/docker/container/run/#ipc) | 🔁 system-mode services get a private PID namespace by default (`PrivatePIDs=yes`, D36); namespace sharing belongs to a compose pod (`JoinsNamespaceOf=` when a case needs IPC/PID), not standalone pair flags ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | ❌ ad-hoc `--ipc`/`--pid`/`--uts` sharing flags; no explicit namespace controls today. |
...
| [`--name`](https://docs.docker.com/reference/cli/docker/container/run/#name) | ⏳ compose members already have stable names; `cix run --name` is deferred mechanical sugar ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | A user-chosen stable handle for one-off runs. |
...
| [`container prune`](https://docs.docker.com/reference/cli/docker/container/prune/) | 🔁 transient units self-collect; `cix clean` removes selected expendable private roles and `cix down --purge` confirms exact cix-owned compose paths | No inventory/preview command or Docker-compatible stopped-container catalog exists. |
...
| docker | disposition | still missing |
| --- | --- | --- |
| [Dockerfile](https://docs.docker.com/reference/dockerfile/) | 🔁 Cixfile v1 + `cix build`, with D47's line-numbered block grammar: named `BUILDER`s feed explicit `SERVICE`, `APP`, and manifest-free `ITEM` artifacts through binders; CIP-91 gives every block one `IMPORT` toolset union; project-local `FROM … OVERLAY` customizes a package universe (D4/D47/D50/D70/CIP-91) + the `.nix` escape hatch | Not an arbitrary Dockerfile frontend: `FROM` binds package universes or source trees rather than filesystem inheritance; org-wide universe trees still use the escape hatch. |
...
| [`RUN`](https://docs.docker.com/reference/dockerfile/#run) | ✅ D47/D51/D71/CIP-87: builder-only, sandboxed, networkless `RUN`, with one-line and heredoc forms; traced regular reads, directory listings, and absent-path probes provide constructive-trace memo hits independent of unrelated workspace changes. Same-step-created paths and their descendants remain outputs rather than inputs, including for PID-namespace children. Networked `FETCH` uses the same read-set cutoff plus output pins, while each builder's disposable underlay preserves incremental tool state; `--cold` audits reads and outputs from empty. | Persistent workspaces and step-output snapshots are host-local and disposable; tracing currently requires Linux ptrace/`strace`. |
| Downloaded FHS-linked native executables | ✅ CIP-95: x86-64 GNU and musl loader aliases are present in every builder and resolve when the matching libc is IMPORTed; failed RUN traces suggest the missing libc IMPORT or name libraries beyond it and teach the patchelf escape | Aliases-only v1 has no `/lib` union. GNU coverage is limited to `DT_NEEDED` libraries supplied by imported glibc; other libraries require explicit patchelf/RPATH surgery. |
| [`COPY`](https://docs.docker.com/reference/dockerfile/#copy) / [`ADD`](https://docs.docker.com/reference/dockerfile/#add) / [`.dockerignore`](https://docs.docker.com/build/concepts/context/#dockerignore-files) | 🔁 CIP-91's one `COPY` materializes source-context paths and links immutable package/builder/FETCH/cix-item paths; descendant role mounts or later writes statically force materialization. Builder COPY steps are sequential and may replace the same destination after a plan edit, including in a warm workspace; final artifact assembly still requires unique destinations. Directory COPY is preferred, scripts remain real copied files, and interpolated `FILE` handles generated content. `LINK` is a temporary deprecated COPY alias. | No URL/tar auto-extracting `ADD`, structural globs, or `.dockerignore` rules. Linked application trees expose their store realpath; a builder `cp` is the escape when a realpath-walking runtime requires materialization. |
| [`ENV`, `EXPOSE`, `VOLUME`, `ENTRYPOINT`, `CMD`, `WORKDIR`, `HEALTHCHECK`, `USER`, `LABEL`](https://docs.docker.com/reference/dockerfile/#overview) | 🔁 `VOLUME` splits honestly: use `STATEDIR` (or another role) for cix-managed private data, and `DIR` for operator-supplied content. Each `SERVICE` or `APP` compiles one bare v0 manifest; compose materializes declared directories as private, shared, or pre-existing host paths. `PORT` is an enforced capability declaration, not metadata; `READINESS`/`LIVENESS` replace exec-shaped `HEALTHCHECK`. | No generic exec health probe, native `LABEL`, arbitrary `USER`, or `WORKDIR`. |
...
| [multi-stage builds](https://docs.docker.com/build/building/multi-stage/) | ✅ multiple named `BUILDER` blocks chain through `COPY ${prev}/ .`; any later artifact can take selected results from any preceding builder, with narrow consumed paths content-address-normalized before linking | References are backward-only, so no cycles or forward-declared graph; there is no Docker layer inheritance. Whole-tree copies and statically required materialization pay their full content cost. |
...
| [reproducible builds](https://docs.docker.com/build/ci/github-actions/reproducible-builds/) | 🔁 Cixfile locks inputs and fixed-output fetches; every declared EXPECT is checked against its exact recorded pin before a completed-output or step memo may return, and `--cold` performs a clean RUN rebuild that audits both traced inputs and selected outputs. The CIP-87 hermetic fixture proves byte-identical convergence, while the deliberately warm-path-dependent proj1 fixture proves read-set divergence is rejected. | Product-wide sampled rebuild scheduling and refusal of every possible impure Nix escape hatch remain absent. |
...
| [`STOPSIGNAL`](https://docs.docker.com/reference/dockerfile/#stopsignal) | ✅ `STOPSIGNAL SIGQUIT` validates known signal names and maps directly to `KillSignal=`; compose `stopTimeout` maps to `TimeoutStopSec=` ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No Dockerfile compatibility beyond this one directive. |
...
| [`docker init`](https://docs.docker.com/reference/cli/docker/init/) | ⏳ the migrate teaching prompt is the generator today; a `cix init` skeleton is later tooling ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No Cixfile, manifest, Nix wrapper, or compose migration skeleton. |
...
## 5. Networking (pod boundary built; named networks remain)

| docker | disposition | still missing |
| --- | --- | --- |
| [port publish / NAT](https://docs.docker.com/engine/network/port-publishing/) | 🔁 Compose publishes a host `.socket`; listeners receive its FD directly across the pod boundary, while fixed-port apps use `systemd-socket-proxyd` in the pod. Bind collisions are rejected during resolution (`scenario-netns`). | DNAT is deliberately not shipped (D49); no Docker port inventory or automatic publish-all surface. |
| [bridge networks and service DNS](https://docs.docker.com/engine/network/drivers/bridge/) | 🔶 `network: "pod"` creates one named netns with shared localhost; an egress-claiming pod gets one networkd-managed veth into cix's fixed bridge and a persisted address. Every sealed root also supplies minimal `localhost` hosts entries, unless its item mounts `/etc/hosts`. | D26 user-defined/named networks, embedded DNS, aliases, cross-composite attachment, and general IPAM objects remain era-parked. |
| [network isolation](https://docs.docker.com/engine/network/) | 🔶 Pod members without egress are address-filtered to loopback; egress members use the stable pod address. Separate pods may reuse the same localhost port, while podless composites retain host networking exactly. | Siblings inside one pod deliberately share ambient localhost. D27 `talks-to` and D26 multi-network membership remain unbuilt. |
...
| [host, none, macvlan, ipvlan network drivers](https://docs.docker.com/engine/network/drivers/) | 🔶 absence of `network: "pod"` is the deliberate host-networking mode; a pod with no egress is loopback-only. | No macvlan/ipvlan drivers or Docker-compatible mode-selection surface. |
| [Compose `network_mode`](https://docs.docker.com/reference/compose-file/services/#network_mode) | 🔶 host is represented by absence of a pod claim; `network: "pod"` gives an explicitly scoped shared namespace, and a pod without egress is loopback-only. | No Docker field compatibility or `service:`/`container:` namespace grafting; pod scope is declared on the group, not selected per leaf. |

## 6. Compose (tree grammar built; deliberate walls remain)

Compose is a root-managed systemd activation path, not a Docker Compose compatibility layer. Its
strict `cixCompose: 1` document is a recursive group tree with deterministic resolution and
generation, one root-owned lock/profile, and `cix up`/`down`/`rollback`; see
`crates/cix-compose/` and the end-to-end
...
`rollback` operate the system manager and require root. Netns/publish climbing, scale, limits,
...
| docker | disposition | still missing |
| --- | --- | --- |
| [Compose services](https://docs.docker.com/reference/compose-file/services/) | 🔁 strict `cixCompose: 1` group nodes use one recursive `children` map of fully-qualified item refs, tagged compose refs, and inline groups. Leaf options apply env/listener/dir/secret/schedule/SHM/stop-timeout/egress policy; group pod claims and recursive publish surfaces are built (`crates/cix-compose/src/{model,resolve,generation}.rs`; `nix/scenarios/{tree,netns,stopdispo}.nix`). | Docker Compose YAML compatibility, multi-file merging/includes, `extends`, device grants, named networks, replicas, and broad Compose-field coverage. |
...
| [`depends_on` / ordering](https://docs.docker.com/reference/compose-file/services/#depends_on) | 🔁 group-local Unix `edges` require their setup unit and put the consumer `After=` the producer service while exposing only declared runtime paths; systemd start-job completion makes structural consumers wait for producer readiness. Published surfaces climb group/pod boundaries explicitly. | ❌ `condition: service_healthy` is deliberately rejected rather than creating a second health graph. Restart propagation and named-network discovery remain. |
| [scale / replicas](https://docs.docker.com/reference/cli/docker/compose/scale/) | ⏳ no replica field or template units | Scaling, stable replica identities, placement, and lifecycle semantics. |
| [`env_file` / secrets](https://docs.docker.com/reference/compose-file/services/#env_file) | 🔁 leaf nodes have explicit non-secret string `env` overrides plus CIP-81 file-only group `secrets: { name: { file\|encrypted } }` → `LoadCredential=` delivery; `SECRET name AS VAR_FILE` supports `_FILE` images | Docker-compatible env-file parsing and raw secret environment delivery (refused). |
...
| [project namespacing](https://docs.docker.com/compose/how-tos/project-name/) | ✅ instance identity is the tree path: `cix-<root>-<path…>.service` units live in matching nested slices beneath `cix-<root>.slice`, with path-keyed role roots/shared surfaces and one root target/profile. Two paths may instantiate one artifact without sharing state (`nix/scenarios/tree.nix`). | Docker's project-name flags and compatibility naming rules. |
| [`up`](https://docs.docker.com/reference/cli/docker/compose/up/) / [`down`](https://docs.docker.com/reference/cli/docker/compose/down/) / rollback | ✅ `cix up` resolves the whole host-root tree and writes one adjacent lock entry per ref path; `--update-lock <path>` repins exactly one subtree. `cix root add/remove` performs atomic structured host-root edits; down and per-root-profile rollback retain their existing semantics (`crates/cix-compose/src/{cli,runtime}.rs`; `nix/scenarios/tree.nix`). | Rootless activation, a long-running reconciler, Docker-compatible lifecycle flags, and cross-host roots. |
...
| [`configs` top-level element](https://docs.docker.com/reference/compose-file/configs/) | ⏳ no compose config object | Top-level config objects, `ConfigurationDirectory=` materialization, ownership/mode, and attachment semantics. |
| [networks, volumes, secrets, configs as reusable top-level objects](https://docs.docker.com/reference/compose-file/) | 🔶 group nodes ship named Unix edges, local `shared:` directory identity, and file-backed credentials; generic reusable objects remain separate work. | Generic external objects, networks, and configs. |
| [`version` marker (obsolete)](https://docs.docker.com/reference/compose-file/version-and-name/#version-top-level-element-obsolete) | 🔁 alpha artifacts carry `cixCompose: 1`, a document-kind schema discriminator; old `composeVersion` is rejected with a `children` migration | Docker's compatibility interpretation and obsolete-version warning behavior. |
...
| [`docker context` / remote hosts](https://docs.docker.com/reference/cli/docker/context/) | ⏳ ssh is the transport; any `cix --host`-style sugar waits ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | Remote-context selection and configuration. |
...
| [Docker Engine API and SDKs](https://docs.docker.com/reference/api/engine/) | ⏳ reconciler-era only; there is no engine API before that ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No stable local/remote API, compatibility policy, or client libraries. |
...
| [Docker Offload](https://docs.docker.com/offload/) | ❌ Nix remote builders are the native answer ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | Docker's paid remote build/run service. |
...
| [capabilities (`--cap-add`)](https://docs.docker.com/engine/containers/run/#runtime-privilege-and-linux-capabilities) | ✅ semantic claims only: declared ports compile to kernel-enforced `SocketBindAllow=`/`SocketBindDeny=`; outside closed-root mode <1024 also grants `NET_BIND_SERVICE`. Closed roots reject direct privileged binds because `PrivateUsers=` makes that capability ineffective in the host network namespace, and teach an unprivileged port or named `LISTENER`. Undeclared bind denial is live-tested in `crates/cix-run/tests/system_projection.rs`. Grow claims case by case, never as raw `--cap-add` passthrough ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)). | No coverage of workloads needing other narrowly scoped capabilities until a dogfood case adds a semantic claim. |
...
| [AppArmor](https://docs.docker.com/engine/security/apparmor/) / [SELinux](https://docs.docker.com/engine/storage/bind-mounts/#configure-the-selinux-label) | 🔁 host security policy, out of manifest scope unless a real SELinux-host user appears ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No manifest labeling/profile behavior for store items and managed writable directories. |
...
| [authorization plugins](https://docs.docker.com/engine/extend/plugins_authorization/) | ❌ no plugin interface; policy returns only with the server/reconciler era ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No present policy-enforcement point or future generic plugin API. |
| [Docker Desktop Enhanced Container Isolation](https://docs.docker.com/enterprise/security/hardened-desktop/enhanced-container-isolation/) | ❌ a desktop-product threat model composix does not share ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | No comparable desktop VM/user-namespace boundary or enterprise control plane. |
...
| [`docker mcp`](https://docs.docker.com/reference/cli/docker/mcp/) | ❌ unrelated to the runtime thesis ([disposition](../cips/dispositions.md#batch-2026-08-04-blessed-by-mathijs-dockermd-application-queued)) | Docker's MCP catalog/toolkit integration. |
