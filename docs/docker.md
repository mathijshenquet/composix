...
| [`image inspect`](https://docs.docker.com/reference/cli/docker/image/inspect/) | ⏳ designed (D35): `cix inspect <ref>` — store path, narHash, per-system outputs, resolved manifest, closure size, signatures, upstream; today the URL page (D18) + `cix ls -l` | `cix inspect` implementation. |
...
| [`docker manifest`](https://docs.docker.com/reference/cli/docker/manifest/) | 🔁 no verb needed (D35): entries are natively per-system (D14), so create/annotate/push have no meaning; visibility via a systems column in `cix ls -l` + `cix inspect` | ❓ The `cix ls -l` systems column is not implemented yet. |
...
| [`container inspect`](https://docs.docker.com/reference/cli/docker/container/inspect/) | ⏳ designed (D35): `cix inspect <unit>` — state, MainPID, exit cause, effective generated properties, port/listener bindings, host role-dir paths; today `cix ps` only lists | `cix inspect` implementation. |
...
| docker | disposition | still missing |
| --- | --- | --- |
| [Dockerfile](https://docs.docker.com/reference/dockerfile/) | 🔁 Cixfile v1 + `cix build`, with line-numbered parse errors and D39's linear `COPY`/`FETCH`/`RUN` chain (D4; `crates/cix-cixfile/src/{parser,build_chain}.rs`) + always the `.nix` escape hatch | Not an arbitrary Dockerfile frontend: `FROM` binds package universes rather than filesystem inheritance, and v0 has no named/multi-stage graph. |
...
| [`RUN`](https://docs.docker.com/reference/dockerfile/#run) | ✅ D39: sandboxed, networkless, offer-only, memoized `RUN`; networked fixed-output `FETCH` is TOFU-pinned in `Cixfile.lock` (`examples/build/projB`) | v0 hashes the full offered closure and whole incoming workdir snapshot; no tracer/pruned read set or named stages yet. |
...
| [multi-stage builds](https://docs.docker.com/build/building/multi-stage/) | 🔁 derivations compose naturally; D39 Cixfile v0 has one linear snapshot chain | No Cixfile stage names, branching, or selective cross-stage copy yet. |
...
| [`RUN --mount=cache/bind/tmpfs`, `--network`, `--security`](https://docs.docker.com/reference/dockerfile/#run) | 🔁 D39 has fixed writable snapshot composition and a keyword-level network contract (`RUN` none, `FETCH` host); package closures are read-only offers | No per-step cache/bind/tmpfs/secret/security flags; writable ecosystem caches live in the workdir snapshot. |
