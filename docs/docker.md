...
| [`ENV`, `EXPOSE`, `VOLUME`, `ENTRYPOINT`, `CMD`, `WORKDIR`, `HEALTHCHECK`, `USER`, `LABEL`](https://docs.docker.com/reference/dockerfile/#overview) | 🔁 each `SERVICE` or `APP` compiles one bare v4 manifest with its grammar-limited contract; `SERVICE` has `PORT`, `LISTENER`, setup, and full role dirs, while `APP` is one-shot; `PORT` is an enforced grant, not metadata (D24/D41/D47/D50) | No native `LABEL`, arbitrary `USER`, `WORKDIR`, or `HEALTHCHECK`; `EXEC` combines entrypoint/command, `EGRESS` awaits netns enforcement, and role dirs are not Docker volume objects. |
