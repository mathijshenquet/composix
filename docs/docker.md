...
| [`RUN`](https://docs.docker.com/reference/dockerfile/#run) | ✅ D47/D51: builder-only, sandboxed, networkless, offer-only, memoized `RUN`, with one-line and heredoc forms; directive continuations and full-line comments retain Docker muscle memory. Networked fixed-output `FETCH` is TOFU-pinned, while builder-scoped `CACHE` directories persist outside memo keys and snapshots (`examples/build/proj1`) | v1 hashes the full offered closure and whole incoming workdir snapshot; no tracer/pruned read set. |
...
| [`ENV`, `EXPOSE`, `VOLUME`, `ENTRYPOINT`, `CMD`, `WORKDIR`, `HEALTHCHECK`, `USER`, `LABEL`](https://docs.docker.com/reference/dockerfile/#overview) | 🔁 each `SERVICE` or `APP` compiles one bare v0 manifest with its grammar-limited contract; `SERVICE` has `PORT`, `LISTENER`, setup, and full role dirs, while `APP` is one-shot; `PORT` is an enforced capability declaration, not metadata (D24/D41/D47/D50) | No native `LABEL`, arbitrary `USER`, `WORKDIR`, or `HEALTHCHECK`; `EXEC` combines entrypoint/command, `CLAIM egress` awaits netns enforcement, and role dirs are not Docker volume objects. |
...
| [`SHELL`, `ONBUILD`, `MAINTAINER`, parser directives](https://docs.docker.com/reference/dockerfile/#parser-directives) | ❌ no global shell directive, image inheritance, downstream triggers, or parser magic; copied scripts name their shell explicitly in `EXEC` (D55) | Per-image default shell choice, inherited downstream triggers, frontend versioning, and direct compatibility with those Dockerfiles. |
| [`RUN --mount=cache/bind/tmpfs`, `--network`, `--security`](https://docs.docker.com/reference/dockerfile/#run) | 🔁 builder-local `CACHE <dir>` persists host-local state across that builder's RUNs but stays outside snapshots/store items; the keyword-level network contract remains `RUN` none, `FETCH` host | No per-step bind/tmpfs/secret/security flags or per-step cache selection. |
