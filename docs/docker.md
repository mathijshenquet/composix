...
| [`-v` / `--mount`](https://docs.docker.com/engine/storage/bind-mounts/) | 🔁 cix-managed private data is a role dir (`STATEDIR`, `CACHEDIR`, `LOGDIR`, or `RUNDIR`); operator-supplied content is `DIR`, materialized by compose. D22 sparse item paths remain read-only projections. | `DIR` materialization (`host:`, `shared:`, `as:`) is compose work. |
