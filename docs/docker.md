...
| docker | disposition | still missing |
| --- | --- | --- |
| [Dockerfile](https://docs.docker.com/reference/dockerfile/) | 🔁 Cixfile v1 + `cix build`, with line-numbered errors, D39's linear `COPY`/`FETCH`/`RUN` chain, and D40 `ITEM`/`TAKE` output boundaries (D4; `crates/cix-cixfile/src/{parser,build_chain}.rs`) + always the `.nix` escape hatch | Not an arbitrary Dockerfile frontend: `FROM` binds package universes rather than filesystem inheritance, and v0 has no named/multi-stage graph. |
| [`FROM` / base images](https://docs.docker.com/reference/dockerfile/#from) | 🔁 `FROM <flakeref> AS <name>` binds a locked package universe for `${name.*}` references; no layer inheritance | ❓ This only helps software packaged in Nix or newly packaged for it; it cannot consume an arbitrary base image. |
| [`RUN`](https://docs.docker.com/reference/dockerfile/#run) | ✅ D39/D40: sandboxed, networkless, offer-only, memoized `RUN`; networked fixed-output `FETCH` is TOFU-pinned, while declared `CACHE` directories persist outside memo keys and snapshots (`examples/build/proj1`) | v0 hashes the full offered closure and whole incoming workdir snapshot; no tracer/pruned read set or named stages yet. |
| [`COPY`](https://docs.docker.com/reference/dockerfile/#copy) / [`ADD`](https://docs.docker.com/reference/dockerfile/#add) / [`.dockerignore`](https://docs.docker.com/build/concepts/context/#dockerignore-files) | 🔁 Cixfile sibling-file `COPY` feeds the build workdir; `TAKE` plucks explicit final files/directories into each ITEM; item-local `FILE`/`SCRIPT` and `LINK` assemble additional content | No Docker-compatible recursive context copy, URL/tar `ADD`, or `.dockerignore` rules. |
| [`ENV`, `EXPOSE`, `VOLUME`, `ENTRYPOINT`, `CMD`, `WORKDIR`, `HEALTHCHECK`, `USER`, `LABEL`](https://docs.docker.com/reference/dockerfile/#overview) | 🔁 each Cixfile `ITEM` compiles one bare v4 manifest with `ENV`, `EXEC`, `PORT`, `LISTENER`, `OUTBOUND`, and role dirs; `PORT` is an enforced grant, not metadata (D24/D40/D41) | No native `LABEL`, arbitrary `USER`, `WORKDIR`, or `HEALTHCHECK`; `EXEC` combines entrypoint/command, `OUTBOUND` awaits netns enforcement, and role dirs are not Docker volume objects. |
...
| [multi-stage builds](https://docs.docker.com/build/building/multi-stage/) | 🔁 derivations compose naturally; D40 can pluck multiple independent items from one final linear snapshot | No Cixfile stage names, branching, or named cross-stage copies. |
