...
| docker | disposition | still missing |
| --- | --- | --- |
| [Dockerfile](https://docs.docker.com/reference/dockerfile/) | 🔁 Cixfile v1 + `cix build`, with D47's line-numbered block grammar: named `BUILDER`s feed explicit `SERVICE`, `APP`, and asset-only `ITEM` artifacts through binders (D4/D47; `crates/cix-cixfile/src/{parser,build_chain}.rs`) + always the `.nix` escape hatch | Not an arbitrary Dockerfile frontend: `FROM` binds package universes or source trees rather than filesystem inheritance. |
...
| [`RUN`](https://docs.docker.com/reference/dockerfile/#run) | ✅ D47: builder-only, sandboxed, networkless, offer-only, memoized `RUN`; networked fixed-output `FETCH` is TOFU-pinned, while builder-scoped `CACHE` directories persist outside memo keys and snapshots (`examples/build/proj1`) | v1 hashes the full offered closure and whole incoming workdir snapshot; no tracer/pruned read set. |
| [`COPY`](https://docs.docker.com/reference/dockerfile/#copy) / [`ADD`](https://docs.docker.com/reference/dockerfile/#add) / [`.dockerignore`](https://docs.docker.com/build/concepts/context/#dockerignore-files) | 🔁 one `COPY` moves bare local-context paths or `${binder}/…` paths into the current builder/artifact; `FILE`/`SCRIPT` and `LINK` assemble additional content | No URL/tar auto-extracting `ADD` or `.dockerignore` rules. |
| [`ENV`, `EXPOSE`, `VOLUME`, `ENTRYPOINT`, `CMD`, `WORKDIR`, `HEALTHCHECK`, `USER`, `LABEL`](https://docs.docker.com/reference/dockerfile/#overview) | 🔁 each `SERVICE` or `APP` compiles one bare v4 manifest with its grammar-limited contract; `SERVICE` has `PORT`, `LISTENER`, setup, and full role dirs, while `APP` is one-shot and `ITEM` has no executable; `PORT` is an enforced grant, not metadata (D24/D41/D47) | No native `LABEL`, arbitrary `USER`, `WORKDIR`, or `HEALTHCHECK`; `EXEC` combines entrypoint/command, `OUTBOUND` awaits netns enforcement, and role dirs are not Docker volume objects. |
