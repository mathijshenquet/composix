...
- **Health is decided, not shipped.** CIP-79 replaces the old generic health bit with
  `READINESS`/`LIVENESS` on systemd notify/watchdog and explicitly bans a separate
  health-gated dependency graph. The directives and prober remain queued.
...
| [`HEALTHCHECK` / health status](https://docs.docker.com/reference/dockerfile/#healthcheck) | ⏳ CIP-79 `READINESS`/`LIVENESS` is adopted; notify/watchdog and the cix http/tcp prober are queued | No health state today. Docker's `condition: service_healthy` graph is deliberately rejected; structural edges will wait for readiness. |
...
| [`--device` / `--gpus` / `--device-cgroup-rule`](https://docs.docker.com/reference/cli/docker/container/run/#device) | ⏳ deliberate (manifest v2 deferral): needs a dogfood case; `DeviceAllow=` exists | Device access today. |
...
| docker | disposition | still missing |
| --- | --- | --- |
| [Dockerfile](https://docs.docker.com/reference/dockerfile/) | 🔁 Cixfile v1 + `cix build`, with D47's line-numbered block grammar: named `BUILDER`s feed explicit `SERVICE` and `APP` artifacts through binders (D4/D47/D50; `crates/cix-cixfile/src/{parser,build_chain}.rs`) + always the `.nix` escape hatch | Not an arbitrary Dockerfile frontend: `FROM` binds package universes or source trees rather than filesystem inheritance. |
...
| [`RUN`](https://docs.docker.com/reference/dockerfile/#run) | ✅ D47/D51/D71: builder-only, sandboxed, networkless, memoized `RUN`, with one-line and heredoc forms; directive continuations and full-line comments retain Docker muscle memory. Networked fixed-output `FETCH` is pinned, while each builder's disposable underlay preserves its previous end state for incremental tools; `--cold` audits from empty. | No tracer/pruned read set; persistent workspaces are host-local and disposable. |
...
| [reproducible builds](https://docs.docker.com/build/ci/github-actions/reproducible-builds/) | 🔁 Cixfile locks inputs and fixed-output fetches; `--no-cache` performs a clean RUN rebuild, with proj1 CI coverage proving byte-identical item paths against the warm-cache build | Product-wide sampled rebuild scheduling and refusal of every possible impure Nix escape hatch remain absent. |
...
| [`depends_on` / ordering](https://docs.docker.com/reference/compose-file/services/#depends_on) | 🔁 v0 Unix `edges` create setup units and consumer `Requires=`/`After=` ordering while exposing only declared runtime paths (`crates/cix-compose/src/generation.rs`; stack `database`/`http` edges) | Restart propagation and network discovery remain. CIP-79 deliberately bans `condition: service_healthy`; structural edges will wait for readiness through systemd start completion. |
...
| [`env_file` / secrets](https://docs.docker.com/reference/compose-file/services/#env_file) | 🔁 v0 has explicit non-secret string `env` overrides; CIP-81 adopts file-only `LoadCredential=` delivery but its implementation is queued | File-based environment loading and secret delivery today. |
...
| [capabilities (`--cap-add`)](https://docs.docker.com/engine/containers/run/#runtime-privilege-and-linux-capabilities) | ✅ semantic claims only: declared ports compile to kernel-enforced `SocketBindAllow=`/`SocketBindDeny=` (and <1024 ⇒ `NET_BIND_SERVICE`); undeclared bind denial is live-tested in `crates/cix-run/tests/system_projection.rs`. Docker has no equivalent port declaration enforcement. | ❓ One implemented semantic claim is not coverage of real workloads that need other narrowly scoped capabilities. |
...
| [secrets](https://docs.docker.com/compose/how-tos/use-secrets/) | ⏳ CIP-81 adopts file-only `LoadCredential=` delivery; implementation is queued | Secret delivery today. |
