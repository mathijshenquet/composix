...
| 2 | Caddy | Prior runtime/cold receipts pass; the assembly rerun's sandbox FETCH returned one 769-byte body for distinct raw assets and stopped before assembly | 🔶 The four-socket translation remains declared, but fresh build evidence awaits a clean builder that can distinguish the pinned GitHub assets | S | [receipt](../corpus/migrate/docker/caddy/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 3 | Directus | The former loader and ENOTDIR walls are cleared; frozen offline install now stops on the stale root lockfile | 🔶 A current package/lock mismatch blocks an item before deploy or probe | M | [build receipt](../corpus/migrate/docker/directus/receipt.md) |
| 4 | Dozzle | Daemonless socket-bridge declaration formats/parses; frontend fetch exceeds the five-minute bound | 🔶 Socket bridge is intentionally desk/unprobed without gate dockerd; no frontend item exists | M | [daemonless receipt](../corpus/migrate/docker/dozzle/receipt.md) |
...
| 7 | Filestash | Independent Cix FETCH stops at DNS resolution before C compilation | 🔶 The static-library loop is not re-established from this DNS-failed receipt; no item exists | L | [build receipt](../corpus/migrate/docker/filestash/receipt.md) |
...
| 17 | Verdaccio | Ordinary monorepo rebuilds produce no item; cold replay has a distinct volatile root read set | 🔶 Runtime layout is translated but unproved without an item; cold volatility is retained rather than repinned | M | [build receipt](../corpus/migrate/docker/verdaccio/receipt.md) |
