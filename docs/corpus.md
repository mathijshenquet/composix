...
|---:|---|---|---|:---:|---|
| 1 | Adminer | Faithful 5.5.0 and dissolved twins; the refreshed faithful login probe passes after its verified source FETCHes move to TOFU pins | 🔶 Full application layout, PHP tuning/extensions, and design/plugin mutation contract regained; the remaining cold replay read-set divergence is at `designs`, optional ENV needs a sentinel, and stop-signal semantics differ | S | [runtime probe + cold read-set defect](../corpus/migrate/docker/adminer/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
