...
|---:|---|---|---|:---:|---|
| 1 | Adminer | Faithful 5.5.0 and dissolved twins; the worker's warm login probe passes and the dissolved twin is cold-stable, while an independent source refetch exposes the unchanged EXPECT mismatch | 🔶 Full application layout, PHP tuning/extensions, and design/plugin mutation contract regained; source FETCH is cold-unstable, and stop-signal semantics differ | S | [runtime probe (worker warm) + independent cold defect](../corpus/migrate/docker/adminer/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
