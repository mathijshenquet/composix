...
|---:|---|---|---|:---:|---|
| 1 | Adminer | Faithful 5.5.0 and dissolved twins; the worker's warm login probe passes and the dissolved twin is cold-stable, while an independent source refetch exposes the unchanged EXPECT mismatch | 🔶 Full application layout, PHP tuning/extensions, and design/plugin mutation contract regained; source FETCH is cold-unstable, optional ENV needs a sentinel, and the case is stale for its now-representable stop signal | S | [runtime probe (worker warm) + independent cold defect](../corpus/migrate/docker/adminer/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
...
| 10 | nginx | Both twins build, but the faithful runtime exits on missing `/var/log/nginx` files; a synthetic nginx response remains sealed-root audited | 🔶 Declared losses — the Alpine hook, Docker log symlinks, fixed uid, and parts of the entrypoint contract differ; the case is stale for its now-representable stop signal | S | [build (runtime failure)](../corpus/migrate/docker/nginx/receipt.md) · [closed-root (synthetic contract)](#cip-84-closed-root-audit) |
