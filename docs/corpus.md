...
| 2 | Caddy | Faithful v2.11.4 config/assets serve HTTP; the package-level responder is separately sealed-root audited | 🔶 Real upstream contract regained — config, welcome page, all four sockets incl. `udp:443`, both twins; the recorded CONFIGDIR and localhost deviations are stale pending regeneration | S | [runtime probe](../corpus/migrate/caddy/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 3 | Directus | Docker probe passes; CIP-95 acceptance proves imported glibc removes the unpatched Sass loader failure, then Cix reaches a separate `Not a directory` failure before producing an item | ⏳ Blocked — FHS-loader support is built; the checked-in conversion needs regeneration and the later path defect remains | M | [build (Cix failure + CIP-95 acceptance)](../corpus/migrate/directus/receipt.md) |
...
| 11 | Parse Server | The staged and independent Mongo-backed health probes pass; independent dependency fetches vary and explicit cold replay diverges at `node_modules` | 🔶 Full release layout, state roles, credentials, JIT, and database behavior regained; Node version and probe port differ, while the recorded CONFIGDIR deviation is stale pending regeneration | M | [runtime probe + cold defect](../corpus/migrate/parse-server/receipt.md) |
...
| 16 | Traefik | Faithful 3.5.6 build answers ping on a warm memo hit; the nixpkgs ping contract is separately sealed-root audited | 🔶 Exact release with digest verification, twins present; entrypoint absent from upstream context, and both FETCH pins deliberately preserve the verified EXPECT-defect repro | S | [runtime probe](../corpus/migrate/traefik/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
...
| 19 | Watchtower | The source-repair build succeeds warm; cold Go-cache comparison and the supplied unit probe remain red | ❌ Refused — useful runtime requires Docker's socket/API and the source build differs from the missing CI binary supply contract | M | [build (warm pass, cold/probe failures)](../corpus/migrate/watchtower/receipt.md) |
