...
|---:|---|---|---|:---:|---|
| 1 | Adminer | Faithful 5.5.0 and dissolved twins; the worker's warm login probe passes and the dissolved twin is cold-stable, while an independent source refetch exposes the unchanged EXPECT mismatch | 🔶 Full application layout, PHP tuning/extensions, and design/plugin mutation contract regained; source FETCH is cold-unstable, optional ENV needs a sentinel, and stop-signal semantics differ | S | [runtime probe (worker warm) + independent cold defect](../corpus/migrate/adminer/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 2 | Caddy | Faithful v2.11.4 config/assets serve HTTP; the package-level responder is separately sealed-root audited | 🔶 Real upstream contract regained — config, welcome page, all four sockets incl. `udp:443`, both twins; residual deviations trace to two recorded cix defects (config-role path, missing `localhost`) | S | [runtime probe](../corpus/migrate/caddy/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 3 | Directus | Docker probe passes; CIP-95 acceptance proves imported glibc removes the unpatched Sass loader failure. The buildfixes scratch replay then crosses the former traced-ENOTDIR recorder failure and reaches the later offline deploy command | ⏳ Stale — the checked-in receipt predates both product fixes; regenerate the conversion before grading its remaining offline deploy boundary | M | [historical build receipt](../corpus/migrate/directus/receipt.md) · [buildfixes scratch receipt](../crates/cix-build/LOG.md) |
| 4 | Dozzle | The full frontend builds, then the backend stops at the source's missing embedded certificates; cold replay also diverges before any item exists | ❌ Refused — useful runtime requires Docker's socket/API; the supplied source is independently incomplete | M | [build (exact warm/cold failures)](../corpus/migrate/dozzle/receipt.md) |
| 5 | Echo Server | The worker's warm build and HTTP probe pass; an independent refetch now rejects the unchanged dependency EXPECT and cannot seed a cold replay | 🔶 Real `/app` contract and HTTP behavior regained; locked Node replaces the moving Alpine image and the npm fetch is cold-unstable | M | [runtime probe (worker warm) + independent cold defect](../corpus/migrate/echo-server/receipt.md) |
| 6 | Excalidraw | The faithful static item builds warm and cold; an in-store runner serves the title on port 80, while the supplied 18090/runner-path harness remains red | 🔶 Upstream layout and native readiness regained; host-architecture selection and two documented volatile-output normalizations differ | M | [warm+cold build and directed runtime proof](../corpus/migrate/excalidraw/receipt.md) |
| 7 | Filestash | Docker probe passes; Cix reaches static-library linkage and fails before an item | ⏳ Blocked — development imports and a coherent static C-library package set are unavailable | L | [build (Cix failure)](../corpus/migrate/filestash/receipt.md) |
| 8 | Memcached | Faithful 1.6.45 source build answers `VERSION`; the nixpkgs protocol contract is separately sealed-root audited | 🔶 Faithful source build at upstream-pinned 1.6.45 with upstream configure flags, twins present; root-only upstream test harness skipped (declared) | S | [runtime probe](../corpus/migrate/memcached/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 9 | NATS | Faithful 2.12.14 binary answers monitoring health; the nixpkgs monitoring contract is separately sealed-root audited | 🔶 Exact upstream release, twins present; the config/entrypoint contract is unrecoverable from SOURCE provenance — the one real loss | S | [runtime probe](../corpus/migrate/nats/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 10 | nginx | Both twins build, but the faithful runtime exits on missing `/var/log/nginx` files; a synthetic nginx response remains sealed-root audited | 🔶 Declared losses — the Alpine hook, Docker log symlinks, fixed uid, stop signal, and parts of the entrypoint contract differ | S | [build (runtime failure)](../corpus/migrate/nginx/receipt.md) · [closed-root (synthetic contract)](#cip-84-closed-root-audit) |
| 11 | Parse Server | The staged and independent Mongo-backed health probes pass; independent dependency fetches vary and explicit cold replay diverges at `node_modules` | 🔶 Full release layout, state roles, credentials, JIT, and database behavior regained; Node version, probe port, and CONFIGDIR path differ | M | [runtime probe + cold defect](../corpus/migrate/parse-server/receipt.md) |
| 12 | phpMyAdmin | The signed 5.2.3 source build passes its login probe and an independent cold replay; the worker's contaminated cold read set is retained as contrasting evidence | 🔶 Config helpers, tuning, sessions, and application layout regained; exact PHP extension build, Apache entrypoint behavior, and secret-file bridges remain losses | S | [warm+cold runtime receipt](../corpus/migrate/phpmyadmin/receipt.md) · [closed-root (login contract)](#cip-84-closed-root-audit) |
| 13 | Redis | Faithful 7.4.8 source build answers PING; the nixpkgs `/data` contract is separately sealed-root audited | 🔶 Full upstream source build retained (protected-mode patch, TLS, jemalloc flags), twins present; identity/`gosu` dissolve into systemd by design | S | [runtime probe](../corpus/migrate/redis/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 14 | Renovate | Both nixpkgs-authoritative twins build warm/cold and the daily calendar/compose contract validates; the sealed-root audit separately proves timer/version/log activation | 🔶 Faithful to the supplied narrow CronJob contract; repository config, credentials, actual renovation, and richer job policy remain absent | S | [build+schedule validation](../corpus/migrate/renovate/receipt.md) · [closed-root (timer/version contract)](#cip-84-closed-root-audit) |
| 15 | Tomcat | Both twins build warm/cold and the empty-server HTTP probe passes; the sealed-root audit proves the same reachability class | 🔶 Tomcat 10.1.57/JRE21 runtime, ports, and twins regained; Tomcat Native and writable work/log trees remain losses | S | [runtime probe](../corpus/migrate/tomcat/receipt.md) · [closed-root](#cip-84-closed-root-audit) |
| 16 | Traefik | The historical faithful 3.5.6 build answers ping on a warm memo hit; current Cix now rejects its deliberately copy-pasted EXPECT against the distinct recorded pin before memo reuse | ⏳ Stale by design — the checked-in case remains the untouched defect repro until the later corpus round normalizes the volatile metadata fetch and records the two correct pins | S | [historical runtime probe](../corpus/migrate/traefik/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 17 | Verdaccio | The full pnpm graph reaches package build/deploy, then fails `Not a directory`; cold replay independently diverges at Corepack state | ⏳ Blocked — runtime layout is substantially translated, but the sandboxed monorepo build still produces no item | M | [build (exact warm/cold failures)](../corpus/migrate/verdaccio/receipt.md) |
| 18 | Wallos | The complete app/state assembly builds cold and the independent `/health.php` probe returns `OK` | 🔶 Main web/state/update behavior is live; cron is omitted, nginx/PHP-FPM remain in-unit, readiness is undeclared, and a probe-only listener is added | M | [runtime probe + cold build](../corpus/migrate/wallos/receipt.md) |
| 19 | Watchtower | The source-repair build succeeds warm; cold Go-cache comparison and the supplied unit probe remain red. The separate repeated-builder-COPY restriction found during conversion is now fixed | ❌ Refused — useful runtime requires Docker's socket/API and the source build differs from the missing CI binary supply contract; regenerate only to remove the stale COPY workaround | M | [build (warm pass, cold/probe failures)](../corpus/migrate/watchtower/receipt.md) |
| 20 | Whoami | Both Docker-shaped and dissolved twins build warm/cold and the HTTP probe passes | 🔶 Runtime certificate/timezone layout and twin regained; absent upstream context leaves the source build unproved | M | [runtime probe + cold twin builds](../corpus/migrate/whoami/receipt.md) |
| 21 | Mastodon | Six-member sealed-root composition: credentials, Unix edges, shared-rw state, health, timer, logs, and purge | 🔶 Declared losses — three application members are stubs and D26/D27 network segmentation is absent | M | [closed-root](#cip-84-closed-root-audit) · [receipt](../corpus/migrate/mastodon/receipt.md) |
