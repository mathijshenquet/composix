...
|---:|---|---|---|:---:|---|
| 1 | Adminer | Base Cix login page under a sealed root; optional entrypoint behavior is outside the probe | 🔶 Declared losses — PHP tuning, upstream layout, and dynamic design/plugin assembly are missing | S | [closed-root](#cip-84-closed-root-audit) · [receipt](../corpus/migrate/adminer/receipt.md) |
| 2 | Caddy | Faithful v2.11.4 config/assets serve HTTP; the package-level responder is separately sealed-root audited | 🔶 Declared deviations — `/config` is forcibly folded into `/data`, a hosts file supplies `localhost`, and HTTP 80 is normalized to 8080; all four sockets and both twins are present | S | [runtime probe](../corpus/migrate/caddy/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 3 | Directus | Docker probe passes; Cix fails before producing an item at an FHS-loader-dependent Sass binary | ⏳ Blocked — downloaded native binary cannot run in the builder; state assembly remains provisional | M | [build (Cix failure)](../corpus/migrate/directus/receipt.md) |
| 4 | Dozzle | Cix UI build is non-reproducible; a backend-only experiment isolates D69 behavior | ❌ Refused — useful runtime requires Docker's socket/API; complete build is also non-green | M | [build (Cix failure)](../corpus/migrate/dozzle/receipt.md) |
| 5 | Echo Server | Historical Cix offline build and bounded HTTP probe | 🔶 Declared losses — upstream layout/tool assembly diverges and closed-root reproduction is pending | M | [runtime probe (historical)](../corpus/migrate/echo-server/receipt.md) |
| 6 | Excalidraw | Historical Docker and Cix title probes | 🔶 Declared losses — build environment/layout differ and Docker health is not declared as readiness | M | [runtime probe (historical)](../corpus/migrate/excalidraw/receipt.md) |
...
| 8 | Memcached | Faithful 1.6.45 source build answers `VERSION`; the nixpkgs protocol contract is separately sealed-root audited | 🔶 Declared deviations — fixed uid is managed, the root-dependent upstream test harness is skipped, and the dissolved twin retains inert upstream metadata | S | [runtime probe](../corpus/migrate/memcached/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 9 | NATS | Faithful 2.12.14 binary answers monitoring health; the nixpkgs monitoring contract is separately sealed-root audited | 🔶 Declared losses — source provenance cannot recover the config/entrypoint contract, the faithful twin is amd64-only, and no message round-trip is probed | S | [runtime probe](../corpus/migrate/nats/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
...
| 11 | Parse Server | Stable pinned Cix item build; Mongo-backed runtime is not exercised | 🔶 Declared losses — config volumes/log path and Node version diverge; runtime remains unproved | M | [build](../corpus/migrate/parse-server/receipt.md) |
| 12 | phpMyAdmin | Sealed-root login-page response | 🔶 Declared losses — PHP extensions/tuning, config generation, session layout, and Docker parity are missing | S | [closed-root](#cip-84-closed-root-audit) · [receipt](../corpus/migrate/phpmyadmin/receipt.md) |
| 13 | Redis | Faithful 7.4.8 source build answers PING; the nixpkgs `/data` contract is separately sealed-root audited | 🔶 Declared deviations — fixed identity and the `gosu` root branch dissolve into systemd; build choices are retained but not exhaustively behavior-probed | S | [runtime probe](../corpus/migrate/redis/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 14 | Renovate | Sealed-root timer activation, `renovate --version`, and indexed logs | 🔶 Declared losses — no repository work, config/credentials, job-policy parity, or faithful twin | S | [closed-root](#cip-84-closed-root-audit) · [receipt](../corpus/migrate/renovate/receipt.md) |
| 15 | Tomcat | Sealed-root empty-server HTTP reachability | 🔶 Declared losses — virtual layout, disabled shutdown listener, application behavior, and faithful twin remain | S | [closed-root](#cip-84-closed-root-audit) · [receipt](../corpus/migrate/tomcat/receipt.md) |
| 16 | Traefik | Faithful 3.5.6 build answers ping on a warm memo hit; the nixpkgs ping contract is separately sealed-root audited | 🔶 Declared losses — the copied entrypoint is unavailable, release-API transport differs, reverse proxying is unprobed, and both FETCH pins preserve verified product defects | S | [runtime probe](../corpus/migrate/traefik/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 17 | Verdaccio | Corepack/pnpm exits non-zero before producing an item | ⏳ Blocked — build diagnostic, runtime layout, and environment parity remain unresolved | M | [build (Cix failure)](../corpus/migrate/verdaccio/receipt.md) |
| 18 | Wallos | Historical Docker and Cix `/health.php` probes through the overlay universe | 🔶 Declared losses — readiness is undeclared, in-unit supervision remains, and closed-root reproduction is pending | M | [runtime probe (historical)](../corpus/migrate/wallos/receipt.md) |
| 19 | Watchtower | Cix source build succeeds; Docker's CI-provided binary is absent | ❌ Refused — useful runtime requires Docker's socket/API and the upstream supply contract is unreproducible | M | [build](../corpus/migrate/watchtower/receipt.md) |
| 20 | Whoami | Historical Cix HTTP probe | 🔶 Declared losses — runtime assets, reproducible closed-root evidence, and dissolved twin are missing | M | [runtime probe (historical)](../corpus/migrate/whoami/receipt.md) |
