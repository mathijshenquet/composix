...
| 27 | Apache HTTPD | Faithful 2.4.68 build and `It works!` HTTP probe pass; dissolved twin builds cold | 🔶 File logging through `LOGDIR` replaces sandbox-inaccessible fd symlinks; the old generated-sed-path cold exhibit is stale under the self-write fix and awaits regeneration | M | [build/runtime and stale cold-read-set receipt](../corpus/migrate/docker/httpd/receipt.md) |
