...
| 3 | Directus | Exact pnpm validates the coherent source; pnpm 11.18 fetches and seals the complete 80,763-file CAS plus `index.db`, and the read-only store remains byte-identical | 🔶 Fetch/seal is verified, but offline install, production deploy, item, and runtime remain unproved | M | [fetch/seal receipt](../corpus/migrate/docker/directus/receipt.md) |
| 4 | Dozzle | Daemonless socket-bridge declaration formats/parses; pnpm 11.18 fetches and seals the complete 20,175-file CAS plus `index.db` | 🔶 The traced offline install exceeded its 20-minute bound; frontend/item/runtime remain unproved, and the socket bridge stays intentionally desk-only | M | [daemonless and frozen-store wall](../corpus/migrate/docker/dozzle/receipt.md) |
...
| 17 | Verdaccio | pnpm 11.17 replays a read-only whole store network-silently twice and produces byte-identical 31-package build output; the actual Cix route seals the same artifact class | 🔶 The traced Cix install exceeded its 20-minute bound, and offline production deploy separately lacks package-mirror metadata; no item/runtime is claimed | M | [frozen-store validation and route walls](../corpus/migrate/docker/verdaccio/receipt.md) |
...
| 26 | HAProxy | Faithful 3.2.22 source build, version probe, and cold replay pass; dissolved twin builds cold | 🔶🔄 No supplied config, listener, or health contract exists to start; CIP-110 fixes formatter key drift and this case awaits the epoch lock regeneration | M | [build/version and formatter receipt](../corpus/migrate/docker/haproxy/receipt.md) |
...
Fifteen migrations remain outside the green closed-root set. Directus now has a
green whole-store fetch/seal but no install or item; Filestash and Verdaccio still
fail before producing a runnable item. Dozzle's whole-store FETCH is diagnosed and
green but its full item is unverified, while its socket bridge and
