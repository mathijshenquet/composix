...
| 16 | Traefik | Faithful 3.5.6 build answers ping on a warm memo hit; the nixpkgs ping contract is separately sealed-root audited | 🔶 Exact release with digest verification, twins present; entrypoint absent from upstream context, and both FETCH pins deliberately preserve the verified EXPECT-defect repro | S | [runtime probe](../corpus/migrate/docker/traefik/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
