...
| 2 | Caddy | Faithful v2.11.4 config/assets serve HTTP; the package-level responder is separately sealed-root audited | 🔶 Real upstream contract regained — config, welcome page, all four sockets incl. `udp:443`, both twins; residual deviations trace to two recorded cix defects (config-role path, missing `localhost`) | S | [runtime probe](../corpus/migrate/docker/caddy/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
