...
| 2 | Caddy | Faithful v2.11.4 config/assets serve HTTP; the package-level responder is separately sealed-root audited | 🔶🔄 Real upstream contract regained — config, welcome page, all four sockets incl. `udp:443`, both twins; the recorded CONFIGDIR and localhost deviations are stale pending the running regeneration | S | [runtime probe](../corpus/migrate/docker/caddy/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 3 | Directus | Docker probe passes; CIP-95 acceptance proves imported glibc removes the unpatched Sass loader failure. The buildfixes scratch replay then crosses the former traced-ENOTDIR recorder failure and reaches the later offline deploy command | 🔶🔄 Stale — the checked-in receipt predates both product fixes; the running regeneration re-grades its remaining offline deploy boundary | M | [historical build receipt](../corpus/migrate/docker/directus/receipt.md) · [buildfixes scratch receipt](../crates/cix-build/LOG.md) |
| 4 | Dozzle | The full frontend builds, then the backend stops at the source's missing embedded certificates; cold replay also diverges before any item exists | 🔶🔄 Socket-bridge rewrite queued (decision 2026-08-05): the Cixfile declares the host docker socket and ships untested against a live daemon; the supplied source remains independently incomplete — the old blanket refusal retires with the rewrite | M | [build (exact warm/cold failures)](../corpus/migrate/docker/dozzle/receipt.md) |
...
| 7 | Filestash | Docker probe passes; Cix reaches static-library linkage and fails before an item | 🔶🔄 Blocked — development imports and a coherent static C-library package set are unavailable; the `pkgsStatic` missing-lib investigation runs in the current wave | L | [build (Cix failure)](../corpus/migrate/docker/filestash/receipt.md) |
...
| 10 | nginx | Both twins build, but the faithful runtime exits on missing `/var/log/nginx` files; a synthetic nginx response remains sealed-root audited | 🔶🔄 Declared losses — the Alpine hook, Docker log symlinks, fixed uid, and parts of the entrypoint contract differ; the case is stale for its now-representable stop signal and the faithful runtime's `/var/log/nginx` failure is regeneration-addressable | S | [build (runtime failure)](../corpus/migrate/docker/nginx/receipt.md) · [closed-root (synthetic contract)](#cip-84-closed-root-audit) |
| 11 | Parse Server | The staged and independent Mongo-backed health probes pass; independent dependency fetches vary and explicit cold replay diverges at `node_modules` | 🔶🔄 Full release layout, state roles, credentials, JIT, and database behavior regained; Node version and probe port differ, while the recorded CONFIGDIR deviation is stale pending the running regeneration | M | [runtime probe + cold defect](../corpus/migrate/docker/parse-server/receipt.md) |
...
| 17 | Verdaccio | The full pnpm graph reaches package build/deploy, then fails `Not a directory`; cold replay independently diverges at Corepack state | 🔶🔄 Blocked — runtime layout is substantially translated, but the sandboxed monorepo build still produces no item; the retry runs in the current wave (its `Not a directory` class is now traced) | M | [build (exact warm/cold failures)](../corpus/migrate/docker/verdaccio/receipt.md) |
| 18 | Wallos | The complete app/state assembly builds cold and the independent `/health.php` probe returns `OK` | 🔶⌛ Main web/state/update behavior is live; cron is omitted, nginx/PHP-FPM remain in-unit, readiness is undeclared, and a probe-only listener is added; the forced `/app` relocation reverts once CIP-98 (artifact-root collision, adopted) is implemented | M | [runtime probe + cold build](../corpus/migrate/docker/wallos/receipt.md) |
| 19 | Watchtower | The source-repair build succeeds warm; cold Go-cache comparison and the supplied unit probe remain red. The separate repeated-builder-COPY restriction found during conversion is now fixed | 🔶🔄 Socket-bridge rewrite queued (decision 2026-08-05): ships untested against a live daemon, the stale COPY workaround disappears in the rewrite, and the source build still differs from the missing CI binary supply contract — the old blanket refusal retires | M | [build (warm pass, cold/probe failures)](../corpus/migrate/docker/watchtower/receipt.md) |
