...
|---:|---|---|---|:---:|---|
| 1 | Adminer | Faithful 5.5.0 and dissolved twins; the refreshed faithful login probe passes after its verified source FETCHes move to TOFU pins | 🔶 Full application layout, PHP tuning/extensions, and design/plugin mutation contract regained; the remaining cold replay read-set divergence is at `designs`, and the case is stale for its now-representable stop signal | S | [runtime probe + cold read-set defect](../corpus/migrate/docker/adminer/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
...
| 5 | Echo Server | The worker's warm build, HTTP probe, and pinned cold replay pass after the script-driven dependency FETCH moves to a TOFU pin | 🔶 Real `/app` contract and HTTP behavior regained; locked Node replaces the moving Alpine image | M | [runtime probe + cold replay](../corpus/migrate/docker/echo-server/receipt.md) |
