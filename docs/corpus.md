...
| 7 | Filestash | Docker probe passes; Cix reaches static-library linkage and fails before an item | ⏳ Blocked — development imports and a coherent static C-library package set are unavailable | L | [build (Cix failure)](../corpus/migrate/docker/filestash/receipt.md) |
...
| 16 | Traefik | The historical faithful 3.5.6 build answers ping on a warm memo hit; current Cix now rejects its deliberately copy-pasted EXPECT against the distinct recorded pin before memo reuse | ⏳ Stale by design — the checked-in case remains the untouched defect repro until the later corpus round normalizes the volatile metadata fetch and records the two correct pins | S | [historical runtime probe](../corpus/migrate/docker/traefik/receipt.md) · [closed-root (package contract)](#cip-84-closed-root-audit) |
| 17 | Verdaccio | The full pnpm graph reaches package build/deploy, then fails `Not a directory`; cold replay independently diverges at Corepack state | ⏳ Blocked — runtime layout is substantially translated, but the sandboxed monorepo build still produces no item | M | [build (exact warm/cold failures)](../corpus/migrate/docker/verdaccio/receipt.md) |
| 18 | Wallos | The complete app/state assembly builds cold and the independent `/health.php` probe returns `OK` | 🔶 Main web/state/update behavior is live; cron is omitted, nginx/PHP-FPM remain in-unit, readiness is undeclared, and a probe-only listener is added | M | [runtime probe + cold build](../corpus/migrate/docker/wallos/receipt.md) |
| 19 | Watchtower | The source-repair build succeeds warm; cold Go-cache comparison and the supplied unit probe remain red. The separate repeated-builder-COPY restriction found during conversion is now fixed | ❌ Refused — useful runtime requires Docker's socket/API and the source build differs from the missing CI binary supply contract; regenerate only to remove the stale COPY workaround | M | [build (warm pass, cold/probe failures)](../corpus/migrate/docker/watchtower/receipt.md) |
| 20 | Whoami | Both Docker-shaped and dissolved twins build warm/cold and the HTTP probe passes | 🔶 Runtime certificate/timezone layout and twin regained; absent upstream context leaves the source build unproved | M | [runtime probe + cold twin builds](../corpus/migrate/docker/whoami/receipt.md) |
| 21 | Mastodon | Six-member sealed-root composition: credentials, Unix edges, shared-rw state, health, timer, logs, and purge | 🔶 Declared losses — three application members are stubs and D26/D27 network segmentation is absent | M | [closed-root](#cip-84-closed-root-audit) · [receipt](../corpus/migrate/docker/mastodon/receipt.md) |
