...
Status: **CIP-82, adopted 2026-08-01** (Mathijs: "kan adopted worden as
is") after four dialogue rounds. Supersedes the binds/shared-rw drafts.
Amends D11. Decision in §5.
...
Adopted as recommended (§3 in full): the claim-unified model with the
undecorated `DIR`; overlay backing with the **full** host mirror; D11's
conventional-root restriction repealed; alias branch and `state-N`
indices deleted; materialization vocabulary `host:`/`shared:`/`as:`
with the stated polarities; the lifecycle table as the normative
contract; recreate refused; `.env` containment **accepted as specified
for now** (own-dir only, generation-identity inclusion, no secrets).
Open-question rulings: idmapped mounts onto foreign-owned host data
require an **explicit acknowledgment** field (silent identity-mapping
is refused); host-mirror alternatives stay dismissed.
...
- 2026-08-02: leg 2 implemented compose host/shared/as materializations, `.env` containment, clean/purge, and run `--dir` parity.
