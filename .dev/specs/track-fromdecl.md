...
1. Parser: `FROM <flakeref> [AS <name>]`, must precede any `${…}` use; missing FROM → error
   ("every Cixfile begins with FROM; try: FROM nixpkgs"). Default namespace `pkgs`; `AS`
   binds others; duplicate namespace → error. Bare `nixpkgs` = the registry ref; otherwise
...
4. Examples: add the one `FROM nixpkgs` line everywhere; one example (node-app) additionally
