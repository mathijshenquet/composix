...
results="$spike/results/go"
session=$(mktemp -d "$spike/work/go-measure.XXXXXX")
mkdir -p "$results"
...
make_work() {
  ...
  cp -a go.mod go.sum main.go "$destination/"
  cp -a "$here/.prepared/vendor/." "$destination/vendor/"
}
...
run_build() {
  ...
  "$harness" "$@" --workdir "$work" --output "$output" --memo "$memo" \
    --trace-prefix "$trace" "${offer_args[@]}" --env "PATH=$path" \
    --env GOFLAGS=-mod=vendor --env GOPROXY=off --env GOSUMDB=off \
    --env CGO_ENABLED=0 --env GOCACHE=/tmp/go-cache \
    -- 'go build -trimpath -o /out/runtrace-go'
}
...
sed -i 's/composix:D38/composix:D39/' "$session/miss/work/main.go"
