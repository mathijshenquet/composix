...
while IFS= read -r source; do
  ...
  case "$source" in
    crates/cix-build/src/build_chain.rs)
      printf '%s\n' "GRANDFATHERED $source: live=$live LOC inline-test=$inline_tests LOC total=$total LOC; the conductor still contains workspace and memo helpers; see cix-build/src/lib.rs module map."
      ;;
    *)
