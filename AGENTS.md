...
- Structural-audit checklist: run `rg -n --pcre2 --glob '*.rs' '(?:\\b(?:Arc|Rc|Mutex|RwLock|RefCell)\\s*(?:<|::)|^\\s*(?:pub(?:\\([^)]*\\))?\\s+)?static\\b)' crates` and review every declaration/use it reports for the required site-local rationale.
