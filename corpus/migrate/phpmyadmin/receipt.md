...
After `bash corpus/migrate/fetch.sh phpmyadmin` exited 0, the assembler observed
a different, clean cold result:

- `target/debug/cix build corpus/migrate/phpmyadmin` exited 0 after verifying
...
- `target/debug/cix build --cold corpus/migrate/phpmyadmin` replayed the pinned
