...
if [ "$mode" = docker ]; then docker build --quiet -t "$name" -f Dockerfile context/mainline/alpine-slim; docker run -d --rm --name "$name" -p 80:80 "$name" >/dev/null; else item=$($CIX build .#nginx); printf 'cix item %s\n' "$item"; unit=$(sudo "$CIX" run --detach "$item" | tail -n1); fi
