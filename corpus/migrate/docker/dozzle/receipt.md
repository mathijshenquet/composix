...
After `bash corpus/migrate/docker/fetch.sh dozzle` exited 0, the assembler reproduced
that boundary twice. `target/debug/cix build corpus/migrate/docker/dozzle` and the
