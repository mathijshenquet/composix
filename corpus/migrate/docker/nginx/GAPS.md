...
- Upstream requests graceful `STOPSIGNAL SIGQUIT`; Cix has no stop-signal declaration, so systemd's stop behavior applies. → language ([recorded stop-signal gap](../../../docs/open-questions.md#proposed-one-line-dispositions-awaiting-mathijs-batch-blessable))
