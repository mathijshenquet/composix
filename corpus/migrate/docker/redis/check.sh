...
if [[ ! -d $root/context ]]; then echo 'context/ missing — run ../../fetch.sh redis first' >&2; exit 1; fi
cleanup() { [ -z "$unit" ] || sudo systemctl stop "$unit" || true; docker rm -f "$name" >/dev/null 2>&1 || true; }; trap cleanup EXIT
if [ "$mode" = docker ]; then docker build --quiet -t "$name" -f Dockerfile context/7.4/alpine; docker run -d --rm --name "$name" -p 6379:6379 "$name" >/dev/null; else item=$($CIX build .#redis); printf 'cix item %s\n' "$item"; unit=$(sudo "$CIX" run --detach "$item" | tail -n1); fi
