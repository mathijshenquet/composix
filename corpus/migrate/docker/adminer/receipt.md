...
Assembler evidence after `bash corpus/migrate/docker/fetch.sh adminer` (exit 0):

- `target/debug/cix build corpus/migrate/docker/adminer` and the identical build in
...
- `target/debug/cix build --file Cixfile.dissolved corpus/migrate/docker/adminer`
  exited 0 with `/nix/store/ayxlixrcrbl2w61s54whwa2d8r4bhm3q-cix-item-adminer`.
- `target/debug/cix build --cold corpus/migrate/docker/adminer` exited 1 because the
