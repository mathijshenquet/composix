...
- The service listens on 18091 for the repository probe rather than Docker's default 1337, and its config role moves from `/parse-server/config` to `/etc/parse-server` because the current runner rejects CONFIGDIR outside `/etc`. → language ([recorded CONFIGDIR restriction](../../../docs/open-questions.md#open-for-agents))
