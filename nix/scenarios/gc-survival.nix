...
scenario.node ''
  ...
  machine.wait_until_succeeds("curl --max-time 5 --fail --silent http://127.0.0.1:18084/ | grep -Fx live:PONG")
  ...
  tagged_api = machine.succeed("CIX_STATE_DIR=/var/lib/cix-index cix inspect scenario-api:track | jq -r .storePath").strip()
  assert tagged_api != current_api
  machine.succeed("nix-store --query --roots " + current_api + " | grep -F /var/lib/cix-compose/gcroots/gc/")
  ...
  machine.succeed("CIX_STATE_DIR=/var/lib/cix-index cix inspect scenario-api:track | grep -F " + tagged_api)
